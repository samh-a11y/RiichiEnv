...
| 模型 | 座样本 | avg_rank | 顺位pt([90,0,-90]) | 原始点差(基差) | 1st% | 2nd% | 3rd% | 和率 | 放铳率 | 立直率 | 立直后和率 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **joint-mse** ×2座 | 60000 | **1.9998** ±.0033 | **+0.013** ±.30 | −0.162 ±.087k | 32.93% | 34.14% | 32.92% | 28.42% | **13.78%** | 23.02% | **53.0%** |
| **v8guard** ×1座 | 30000 | **2.0003** ±.0048 | **−0.027** ±.43 | +0.288 ±.127k | 34.13% | 31.71% | 34.16% | 29.23% | 15.42% | 27.35% | 51.4% |
...
- **原始点差（基差=score−35000，看分差大小不看名次）：v8guard 微正 +0.288k(~2σ)、joint 微负 −0.162k**（零和自洽 2×−0.162+0.288≈0）。⚠ 注意：原始点差与顺位 pt **可反向**——顺位 1 位封顶 +90 不论赢多少，原始点差给全额。v8guard 激进→赢得大但也更常掉 3 位（1st/3rd 双高），故**单局多收 ~288 点却没换成更好名次**。是「赢得大但不稳」，非矛盾。
- **风格对比**（去座位混淆）：v8guard = 激进（立直 27.4% vs 23.0%、和率 29.2% vs 28.4%、放铳 15.4% vs 13.8%、1st/3rd 双高=波动大）；joint-mse = 稳健（放铳最低 13.8%、2nd 最多 34.1%、立直后和率更高 53.0%）。延续 change-001「joint 稳/v8 凶、强弱极接近」的格局——guard 加在 v8 上 + joint 换在线 mse 权重后，**名次平手；仅原始点数 v8guard 微弱占优（不转化为名次）**。
