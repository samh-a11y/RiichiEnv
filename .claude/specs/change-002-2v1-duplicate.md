...
| 模型 | 座样本 | avg_rank | 顺位pt([90,0,-90]) | 素点(归还立直棒,零和) | 1st% | 2nd% | 3rd% | 和率 | 放铳率 | 立直率 | 立直后和率 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **joint-mse** ×2座 | 60000 | **1.9998** ±.0033 | **+0.013** ±.30 | **−0.151k** ±.087 | 32.93% | 34.14% | 32.92% | 28.42% | **13.78%** | 23.02% | **53.0%** |
| **v8guard** ×1座 | 30000 | **2.0003** ±.0048 | **−0.027** ±.43 | **+0.301k** ±.127 | 34.13% | 31.71% | 34.16% | 29.23% | 15.42% | 27.35% | 51.4% |

- **名次维度（avg_rank + 官方 avg_pt=顺位点[90,0,-90]）实质平手**：joint 1.9998 / v8guard 2.0003，差 0.0005 << 噪声。顺位 pt 与 avg_rank 严格同向自洽（`avg_pt=−90·(avg_rank−2)`，核验吻合），joint 微弱领先但不显著。
- **素点维度：v8guard 占优 +0.301k(~2.4σ)、joint −0.151k，零和闭合**（2×−0.151+0.301=0.000）。⚠ 立直棒漏点修正：`RiichiEnv.scores` 终局**不把桌上未回收的立直棒归还 1 位**（962/30000=3.2% 局，多为终局流局留棒、仅 27 局真击飞），故原始三家和 < 105000、平均缺 36 点/局 → 原始素点 −0.162/+0.288 **非零和**（2×−0.162+0.288=−0.036=漏点）。按规则把剩棒归还 1 位后才零和（上表数值）。
- **素点与顺位 pt 可反向**（v8guard 名次略差却素点占优）：顺位 1 位封顶 +90 不论赢多少，素点给全额。v8guard 激进→赢得大但也更常掉 3 位（1st/3rd 双高），故**单局多收 ~300 点却没换成更好名次**。是「赢得大但不稳」，非矛盾。
- **风格对比**（去座位混淆）：v8guard = 激进（立直 27.4% vs 23.0%、和率 29.2% vs 28.4%、放铳 15.4% vs 13.8%、1st/3rd 双高=波动大）；joint-mse = 稳健（放铳最低 13.8%、2nd 最多 34.1%、立直后和率更高 53.0%）。延续 change-001「joint 稳/v8 凶、强弱极接近」的格局——guard 加在 v8 上 + joint 换在线 mse 权重后，**名次平手；仅素点 v8guard 微弱占优(+0.30k/局,不转化为名次)**。
