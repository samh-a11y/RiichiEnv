...
- [ ] gpu-16 资产就位 + smoke 通过。
- [ ] 30k 跑完，出 2v1 强弱（avg_rank/avg_pt + 细分指标）。

## 续点
- 资产传输中（gpu-16）。传完：解压拼装 v8_bc → gpu-16 smoke 1-2 局 → 验证 Stat 同名聚合 → nohup 跑 30k → stat_report 出强弱。
- danger head 当前用 `v8_danger`；若用户要 `v8_danger_suit4` 改 registry 的 `ARENA_V8_DANGER` 一行。
