...
- **🐛 c08 红5 aka 修复**：`possible_actions` 把红5 dahai 折叠成普通5（实测），旧 `_match` 严格比 pai ⇒ bot 切/摸切红5被误判非法回退＝「模型不认识红5」。修：`_deaka` 归一化，命中后原样发 bot 的 `5pr`（`be303c7`）。
- **🔧 c08 end_game 立即重排**：`reconnect_sec` 默认 3→**0**，异常走 `reconnect_backoff_sec=3.0` 退避防猛捶（`aac635c`）。
- **🔧 c08 level/aggr 剥削旋钮接入 CLI**：aux 水平/激进度＝条件于对手 profile 剥削（三家同值）；加 `--level/--aggr`（None＝默认 8.0/12.0）+ `live_start.sh` `LEVEL=/AGGR=` 透传（`be303c7`）。
- 回归测试 `online3p/test_action_match.py` **13/13 PASS**（ankan + 红5 + 无回归）。pt 调参文档见 README「pt 目标函数」节（`7c577bd`）。⚠ **以上全部需 `live_stop→live_start` 重启才生效**。详见 `sessions/c08.md`。
